...
check-safety:
	...
	$(BANDIT_COMMAND_FLAG)poetry run bandit -ll -r overhood/
...
check-style:
	...
	$(MYPY_COMMAND_FLAG)poetry run mypy --config-file setup.cfg overhood tests/**/*.py
...
# Example: make docker IMAGE=some_name VERSION=0.9.0
.PHONY: docker
docker:
...
# Example: make clean_docker IMAGE=some_name VERSION=0.9.0
.PHONY: clean_docker
clean_docker:
